...
BUS_CONFIGS = {
    config.base_can: {
        "motor_types": [oa.MotorType.DM8009, oa.MotorType.DM8009, oa.MotorType.DM8009, oa.MotorType.DM8009,
        ...
        "control_modes": [oa.ControlMode.POS_VEL, oa.ControlMode.POS_VEL, oa.ControlMode.POS_VEL, oa.ControlMode.POS_VEL,
                          oa.ControlMode.VEL, oa.ControlMode.VEL, oa.ControlMode.VEL, oa.ControlMode.VEL ],
    },
    config.left_arm_can: {
...
class HardwareNode:
        ...
        for can_interface, can_config in BUS_CONFIGS.items():
            ...
            if can_interface == config.base_can:
                for i in range(8):
                    ...
                    can_config["control_modes"][id-1] = oa.ControlMode.VEL if i % 2 else oa .ControlMode.POS_VEL
    ...
    def control_loop(self):
        ...
        base.flush_rx()
        for i in range(8):
            self.lowstate.motor_state[i].q  = base_motors[config.base_ids[i]-1].get_position() * config.base_direction[i]
            self.lowstate.motor_state[i].dq = base_motors[config.base_ids[i]-1].get_velocity() * config.base_direction[i]
            self.lowstate.motor_state[i].tau_est = base_motors[config.base_ids[i]-1].get_torque() * config.base_direction[i]
        ...
        for i, motor in enumerate(left_arm.get_arm().get_motors()):
            ...
            self.lowstate.motor_state[8+i].dq = motor.get_velocity() * config.left_arm_direction[i]
            self.lowstate.motor_state[8+i].tau_est = motor.get_torque() * config.left_arm_direction[i]

        right_arm = self.group.get_openarm(config.right_arm_can)
        right_arm.flush_rx()
        for i, motor in enumerate(right_arm.get_arm().get_motors()):
            self.lowstate.motor_state[16+i].q  = motor.get_position() * config.right_arm_direction[i]
            self.lowstate.motor_state[16+i].dq = motor.get_velocity() * config.right_arm_direction[i]
            self.lowstate.motor_state[16+i].tau_est = motor.get_torque() * config.right_arm_direction[i]
